...
func buildVendorSnippet(vendor string, kpis []schema.KPIDefinition) string {
	...
	for _, kpi := range kpis {
		...
		sb.WriteString(fmt.Sprintf("        source_template: \"%s:{{.Job}}:{{.Reader}}:REPLACE_WITH_COUNTER_NAME:{{.Instance}}\"\n", vendor))
		sb.WriteString(fmt.Sprintf("        type: %s\n", metricType))
		sb.WriteString(fmt.Sprintf("        reset_aware: %s\n", resetAware))
...
func generateScenario(vendor, schemaDir string) (string, error) {
	...
	var sb strings.Builder
	sb.WriteString(fmt.Sprintf("name: %s_amf_alarm_storm\n", vendor))
	sb.WriteString(fmt.Sprintf("description: Registration storm scenario — %s vendor metrics\n", vendor))
	...
	sb.WriteString("    metrics:\n")
	sb.WriteString(fmt.Sprintf("      - name: \"%s:job_1:reader_1:REPLACE_REG_ATTEMPTS:amf_001\"\n", vendor))
	sb.WriteString("        labels: {}\n")
	sb.WriteString("        type: counter\n")
	sb.WriteString("        baseline: 10000\n")
	sb.WriteString("        rate_per_second: 15\n")
	sb.WriteString(fmt.Sprintf("      - name: \"%s:job_1:reader_1:REPLACE_REG_FAILURES:amf_001\"\n", vendor))
	sb.WriteString("        labels: {}\n")
	sb.WriteString("        type: counter\n")
	sb.WriteString("        baseline: 30\n")
	sb.WriteString("        rate_per_second: 0.05\n")
	sb.WriteString(fmt.Sprintf("      - name: \"%s:job_1:reader_1:REPLACE_UE_CONNECTED:amf_001\"\n", vendor))
	sb.WriteString("        labels: {}\n")
	...
	sb.WriteString(fmt.Sprintf("        metric: \"%s:job_1:reader_1:REPLACE_REG_ATTEMPTS:amf_001\"\n", vendor))
	sb.WriteString("        rate_scale: 50\n")
	sb.WriteString("      - name: rejection_spike\n")
	sb.WriteString("        start_sec: 60\n")
	sb.WriteString("        duration_sec: 120\n")
	sb.WriteString(fmt.Sprintf("        metric: \"%s:job_1:reader_1:REPLACE_REG_FAILURES:amf_001\"\n", vendor))
	sb.WriteString("        rate_scale: 10000\n")
	sb.WriteString("      - name: ue_drop\n")
	sb.WriteString("        start_sec: 60\n")
	sb.WriteString("        duration_sec: 120\n")
	sb.WriteString(fmt.Sprintf("        metric: \"%s:job_1:reader_1:REPLACE_UE_CONNECTED:amf_001\"\n", vendor))
